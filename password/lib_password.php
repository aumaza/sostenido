...
public function resetPassword($onePassword,$user,$password_1,$password_2,$conn,$dbname){
	...
	$sql = "select user from sos_usuarios where user = $onePassword->getUser('$user')";
	...
	if($rows == 1){
		...
		if($validatePass == 0){
			...
			$sql_1 = "update sos_usuarios set password = $onePassword->setPassword('$passHash') where user = $onePassword->getUser('$user')";
